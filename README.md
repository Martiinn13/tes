...
* graph TD
    subgraph "STATSports Apex - Hardwarová jednotka"
        A[Li-ion Baterie] --> B[PMIC - Řízení napájení]
        B --> C[Mikrokontrolér ARM Cortex-M]
        
        subgraph "Senzorová vrstva"
            D[GNSS Modul - GPS/GLONASS] --> C
            E[IMU - Akcelerometr + Gyroskop] --> C
            F[Magnetometr - Digitální kompas] --> C
        end
        
        C --> G[Vnitřní Flash paměť - Data Logging]
        C --> H[Bluetooth Low Energy - Synchronizace]
        C --> I[USB rozhraní - Nabíjení/Data]
    end
...
def simulate_sampling():
    # Simulace trasy sprintu s prudkou zatáčkou (10 sekund)
    t = np.linspace(0, 10, 100)  # Reálný spojitý pohyb
    x = t
    y = np.sin(t) * 2  # Vlnovka simulující kličkování na hřišti

    # 1. Profesionální vesta (10 Hz) - zachytí téměř vše
    t_10hz = np.linspace(0, 10, 100)
    x_10hz = t_10hz
    y_10hz = np.sin(t_10hz) * 2

    # 2. Apple Watch (1 Hz) - ztrácí detaily v zatáčkách
    t_1hz = np.linspace(0, 10, 10)
    x_1hz = t_1hz
    y_1hz = np.sin(t_1hz) * 2

    # Výpočet "naměřené" vzdálenosti
    dist_10hz = np.sum(np.sqrt(np.diff(x_10hz)**2 + np.diff(y_10hz)**2))
    dist_1hz = np.sum(np.sqrt(np.diff(x_1hz)**2 + np.diff(y_1hz)**2))

    print(f"Vzdálenost (Vesta 10Hz): {dist_10hz:.2f} m")
    print(f"Vzdálenost (Apple Watch 1Hz): {dist_1hz:.2f} m")
    print(f"Ztráta přesnosti: {((dist_10hz - dist_1hz) / dist_10hz) * 100:.1f} %")

    # Vykreslení grafu
    plt.figure(figsize=(10, 5))
    plt.plot(x, y, 'k--', label='Reálná trasa fotbalisty', alpha=0.3)
    plt.plot(x_10hz, y_10hz, 'b-o', label='STATSports (10 Hz)', markersize=4)
    plt.plot(x_1hz, y_1hz, 'r-s', label='Apple Watch (1 Hz)', markersize=8)
    
    plt.title("Srovnání vzorkovací frekvence: 10 Hz vs 1 Hz")
    plt.xlabel("Vzdálenost X [m]")
    plt.ylabel("Vzdálenost Y [m]")
    plt.legend()
    plt.grid(True)
    plt.show()
...
if __name__ == "__main__":
    ...
    ## 7. Softwarová simulace vzorkování
V rámci projektu jsem vytvořil Python skript `data_analysis.py`, který demonstruje, proč dochází k odchylkám v měření vzdálenosti. 

Při simulaci pohybu s kličkováním (sinusoida) skript vypočítal:
* **Vesta (10 Hz):** Zachycuje detailní trajektorii.
* **Hodinky (1 Hz):** Spojují body úsečkami, čímž dochází ke zkreslení celkové uběhnuté vzdálenosti.

Tento kód lze spustit v prostředí Python 3 s knihovnami `numpy` a `matplotlib`.
